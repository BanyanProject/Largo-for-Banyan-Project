...
/*
 * Filter Comments by Top Term
 */ 

add_filter( 'comments_clauses', array('BP_Curated_Comments' , 'bp_comments_filter'));

class BP_Curated_Comments
{
     ...
     * @return array
     */
    public static function bp_comments_filter( $q )
    {
...
/**
 * Load the comment template specified in $file. Overrides comments_template()
 */
function bp_comments_template($term_id = null, $file = '/comments.php', $separate_comments = false ) {
	...
	$comments = BP_Curated_Comments::get_comments($term_id, $comment_args );
 ...
 */
if (!function_exists('bp_comment_form'))
{
...
/*
 * Edit-Comments Table and Quick Edit 
 */
...
add_filter('comment_save_pre', 'bp_save_comment' );

function bp_save_comment($comment_content) {
    global $wpdb;
    $data = array();
...
add_action( 'add_meta_boxes', 'add_edit_comments_meta_boxes');

function add_edit_comments_meta_boxes() {
...
function ajax_comment_upvote_handler() {
...
add_action( 'wp_ajax_comment_upvote_handler', 'ajax_comment_upvote_handler' );
...
function ajax_comment_alert_handler() {
...
add_action( 'wp_ajax_comment_alert_handler', 'ajax_comment_alert_handler' );


/*
 * Respond to posted comments with AJAX
 */

function ajax_comment_handler($comment_ID, $comment_status) {
...
add_action('comment_post', 'ajax_comment_handler', 100, 2);
...
/*
 * Handles comment posts from category pages
 * 
 * Called by comment_id_not_found action
 */

function process_category_comment($comment_post_ID) {
...
add_action('comment_id_not_found', 'process_category_comment', 100, 2);
