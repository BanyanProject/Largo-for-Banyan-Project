	...
	<?php
		if ( have_posts() || largo_have_featured_posts() ) {
			...
			if ( is_author() ) {
				...
				if ( is_month() ) {
					$title = sprintf( __( 'Archives: <span>%s</span>', 'largo' ), get_the_date( 'F Y' ) );
				} elseif ( is_year() ) {
					$title = sprintf( __( 'Archives: <span>%s</span>', 'largo' ), get_the_date( 'Y' ) );
				} else {
		...
		<header class="archive-background clearfix">
			...
			<?php
				if ( isset( $rss_link ) ) {
					printf( '<a class="rss-link rss-subscribe-link" href="%1$s">%2$s</a>', $rss_link, __( 'Subscribe', 'largo' ) );
				}
		...
		<div class="row row-fluid clearfix">
			...
			<?php
				...
				while ( have_posts() ) : the_post();
					get_template_part( 'partials/content', 'teaser-tags' );
				endwhile;
